...
class PostManager {
    constructor() {
        this.posts = [];
        this.parser = new core_1.MarkdownParser();
    }
    /**
     * 加载所有文章
     */
    async loadPosts(postsDir) {
        utils_1.logger.info(`Loading posts from: ${postsDir}`);
        try {
            // 获取所有Markdown文件
            const files = await file_1.FileUtils.getMarkdownFiles(postsDir);
            utils_1.logger.info(`Found ${files.length} markdown files`);
            // 解析所有文章
            const posts = [];
            for (const file of files) {
                try {
                    ...
                    if (!post.draft) {
                        ...
                        utils_1.logger.debug(`Skipped draft post: ${post.title}`);
                    }
                }
                catch (error) {
                    utils_1.logger.error(`Failed to parse post: ${file}`, error);
                }
            }
            // 按日期排序（最新的在前）
            ...
            utils_1.logger.error('Failed to load posts', error);
            throw error;
        }
    }
    /**
    ...
    getAdjacentPosts(currentPost) {
        ...
        return {
            ...
            next: currentIndex < this.posts.length - 1
     ...
     * 获取相关文章
     */
    getRelatedPosts(currentPost, limit = 5) {
...
//# sourceMappingURL=post-manager.js.map