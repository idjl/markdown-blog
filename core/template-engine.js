...
class TemplateEngine {
     ...
     */
    async loadTemplates() {
        const templatesDir = path_1.default.resolve(process.cwd(), this.config.build?.templatesDir || 'src/templates');
        logger_1.logger.debug(`Looking for templates in: ${templatesDir}`);
        try {
            const pattern = path_1.default.join(templatesDir, '*.html');
            logger_1.logger.debug(`Using glob pattern: ${pattern}`);
            const templateFiles = await utils_1.FileUtils.glob(pattern);
            logger_1.logger.debug(`Found ${templateFiles.length} template files:`, templateFiles);
            for (const file of templateFiles) {
                const templateName = path_1.default.basename(file, '.html');
                const content = await utils_1.FileUtils.readFile(file);
                this.templates.set(templateName, content);
                logger_1.logger.debug(`Loaded template: ${templateName}`);
            }
            logger_1.logger.success(`Loaded ${templateFiles.length} templates`);
        }
        catch (error) {
            logger_1.logger.error('Failed to load templates', error);
            // 使用默认模板
            this.loadDefaultTemplates();
        }
    }
    ...
    loadDefaultTemplates() {
        ...
        logger_1.logger.info('Loaded default templates');
    }
    /**
     * 渲染模板
     */
