...
class StaticGenerator {
    constructor(config) {
        this.config = config;
        this.postManager = new core_1.PostManager();
        this.templateEngine = new core_1.TemplateEngine(config);
    }
    /**
     * 生成静态网站
     */
    async generate() {
        utils_1.logger.info('Starting static site generation...');
        try {
            // 1. 加载文章
            const posts = await this.loadPosts();
            // 2. 处理数据
            const { categories, tags, archives } = core_1.DataProcessor.processPosts(posts);
            // 3. 生成页面
            ...
            utils_1.logger.success('Static site generation completed!');
            return {
                posts,
            ...
            utils_1.logger.error('Static site generation failed', error);
            throw error;
        }
    }
    /**
     ...
     */
    async loadPosts() {
        utils_1.logger.info('Loading posts...');
        const postsDir = path_1.default.resolve(process.cwd(), this.config.posts?.dir || 'posts');
        const posts = await this.postManager.loadPosts(postsDir);
        utils_1.logger.success(`Loaded ${posts.length} posts`);
        return posts;
    }
    /**
     * 生成所有页面
     */
    async generatePages(posts, categories, tags, archives) {
        utils_1.logger.info('Generating pages...');
        // 生成首页
        ...
        utils_1.logger.success('Pages generated successfully');
    }
    /**
     * 生成首页
     */
    async generateHomePages(posts) {
        utils_1.logger.info('Generating home pages...');
        const homePages = core_1.DataProcessor.generateHomePages(posts, this.config.posts.perPage);
        for (const pageData of homePages) {
            const templateData = {
                ...
                pagination: {
                    ...
                    nextUrl: pageData.page < pageData.totalPages
                        ? `/page/${pageData.page + 1}/`
                        : undefined,
                    prevUrl: pageData.page > 1
                        ? pageData.page === 2
                            ? '/'
                            : `/page/${pageData.page - 1}/`
                        : undefined,
            ...
            const filePath = pageData.page === 1
                ? path_1.default.join(this.config.build.outputDir, 'index.html')
                : path_1.default.join(this.config.build.outputDir, 'page', String(pageData.page), 'index.html');
            await utils_1.FileUtils.writeFile(filePath, html);
            utils_1.logger.debug(`Generated home page ${pageData.page}`);
        }
    }
    /**
     * 生成文章页面
     */
    async generatePostPages(posts) {
        utils_1.logger.info('Generating post pages...');
        for (const post of posts) {
            ...
            const filePath = path_1.default.join(this.config.build.outputDir, 'posts', post.slug, 'index.html');
            await utils_1.FileUtils.writeFile(filePath, html);
            utils_1.logger.debug(`Generated post page: ${post.slug}`);
        }
    }
    /**
     * 生成分类页面
     */
    async generateCategoryPages(categories) {
        utils_1.logger.info('Generating category pages...');
        const categoryPages = core_1.DataProcessor.generateCategoryPages(categories, this.config.posts.perPage);
        for (const pageData of categoryPages) {
            ...
            const filePath = pageData.page === 1
                ? path_1.default.join(this.config.build.outputDir, 'categories', pageData.category.slug, 'index.html')
                : path_1.default.join(this.config.build.outputDir, 'categories', pageData.category.slug, 'page', String(pageData.page), 'index.html');
            await utils_1.FileUtils.writeFile(filePath, html);
            utils_1.logger.debug(`Generated category page: ${pageData.category.slug} - page ${pageData.page}`);
        }
    }
    /**
     * 生成标签页面
     */
    async generateTagPages(tags) {
        utils_1.logger.info('Generating tag pages...');
        const tagPages = core_1.DataProcessor.generateTagPages(tags, this.config.posts.perPage);
        for (const pageData of tagPages) {
            ...
            const filePath = pageData.page === 1
                ? path_1.default.join(this.config.build.outputDir, 'tags', pageData.tag.slug, 'index.html')
                : path_1.default.join(this.config.build.outputDir, 'tags', pageData.tag.slug, 'page', String(pageData.page), 'index.html');
            await utils_1.FileUtils.writeFile(filePath, html);
            utils_1.logger.debug(`Generated tag page: ${pageData.tag.slug} - page ${pageData.page}`);
        }
    }
    /**
     * 生成归档页面
     */
    async generateArchivePages(archives) {
        utils_1.logger.info('Generating archive pages...');
        const archivePages = core_1.DataProcessor.generateArchivePages(archives, this.config.posts.perPage);
        for (const pageData of archivePages) {
            ...
            const filePath = pageData.page === 1
                ? path_1.default.join(this.config.build.outputDir, 'archives', String(pageData.archive.year), String(pageData.archive.month), 'index.html')
                : path_1.default.join(this.config.build.outputDir, 'archives', String(pageData.archive.year), String(pageData.archive.month), 'page', String(pageData.page), 'index.html');
            await utils_1.FileUtils.writeFile(filePath, html);
            utils_1.logger.debug(`Generated archive page: ${pageData.archive.year}-${pageData.archive.month} - page ${pageData.page}`);
        }
    }
    /**
     * 生成搜索页面
    ...
    async generateSearchPage() {
        utils_1.logger.info('Generating search page...');
        const templateData = {
            title: '搜索',
            content: '',
            meta: {
        ...
        const filePath = path_1.default.join(this.config.build.outputDir, 'search', 'index.html');
        await utils_1.FileUtils.writeFile(filePath, html);
        utils_1.logger.debug('Generated search page');
    }
    /**
     * 生成404页面
     */
    async generate404Page() {
        utils_1.logger.info('Generating 404 page...');
        const templateData = {
            title: '页面未找到',
            content: '',
            meta: {
        ...
        const filePath = path_1.default.join(this.config.build.outputDir, '404.html');
        await utils_1.FileUtils.writeFile(filePath, html);
        utils_1.logger.debug('Generated 404 page');
    }
    /**
     * 复制静态资源
     */
    async copyAssets() {
        ...
        const assetsDir = path_1.default.resolve(process.cwd(), 'src/assets');
        const outputAssetsDir = path_1.default.join(this.config.build.outputDir, 'assets');
        // 复制public目录
        if (await utils_1.FileUtils.exists(publicDir)) {
            await utils_1.FileUtils.copy(publicDir, this.config.build.outputDir);
            utils_1.logger.debug('Copied public assets');
        }
        // 复制src/assets目录
        if (await utils_1.FileUtils.exists(assetsDir)) {
            await utils_1.FileUtils.copy(assetsDir, outputAssetsDir);
            utils_1.logger.debug('Copied source assets');
        }
        // 生成CSS和JS文件
        await this.generateAssets();
        utils_1.logger.success('Assets copied successfully');
    }
    /**
     * 生成资源文件
     */
    ...
    async generateCSS(outputDir) {
        ...
        await utils_1.FileUtils.writeFile(path_1.default.join(cssDir, 'main.css'), mainCSS);
        // Prism.js样式文件
        const prismCSS = this.generatePrismCSS();
        await utils_1.FileUtils.writeFile(path_1.default.join(cssDir, 'prism.css'), prismCSS);
        utils_1.logger.debug('Generated CSS files');
    }
    /**
     * 生成主CSS
     */
...
`;
     ...
     * 生成JavaScript文件
     */
    async generateJS(outputDir) {
        const jsDir = path_1.default.join(outputDir, 'js');
        await utils_1.FileUtils.ensureDir(jsDir);
        // 主JavaScript文件
        const mainJS = this.generateMainJS();
        await utils_1.FileUtils.writeFile(path_1.default.join(jsDir, 'main.js'), mainJS);
        // Prism.js文件
        const prismJS = this.generatePrismJS();
        await utils_1.FileUtils.writeFile(path_1.default.join(jsDir, 'prism.js'), prismJS);
        // 搜索功能
        const searchJS = this.generateSearchJS();
        await utils_1.FileUtils.writeFile(path_1.default.join(jsDir, 'search.js'), searchJS);
        utils_1.logger.debug('Generated JavaScript files');
    }
    /**
     * 生成主JavaScript
     */
...
`;
    ...
    async generateCNAME() {
        if (this.config.url && !this.config.url.includes('github.io')) {
            const cnameContent = this.config.url
                .replace(/^https?:\/\//, '')
                .replace(/\/$/, '');
            const cnamePath = path_1.default.join(this.config.build.outputDir, 'CNAME');
            await utils_1.FileUtils.writeFile(cnamePath, cnameContent);
            utils_1.logger.debug('Generated CNAME file');
        }
    }
    /**
     * 生成RSS订阅
    ...
    async generateRSS(posts) {
        ...
        utils_1.logger.info('Generating RSS feed...');
        const { Feed } = await Promise.resolve().then(() => __importStar(require('feed')));
        const feed = new Feed({
        ...
        const rssPath = path_1.default.join(this.config.build.outputDir, 'rss.xml');
        await utils_1.FileUtils.writeFile(rssPath, rssContent);
        utils_1.logger.success('RSS feed generated');
        return rssPath;
    }
    /**
     * 生成站点地图
     */
    async generateSitemap(posts, categories, tags, archives) {
        if (!this.config.plugins.sitemap.enabled) {
            return '';
        }
        utils_1.logger.info('Generating sitemap...');
        const { SitemapStream } = await Promise.resolve().then(() => __importStar(require('sitemap')));
        const { createWriteStream } = await Promise.resolve().then(() => __importStar(require('fs')));
        const sitemapPath = path_1.default.join(this.config.build.outputDir, 'sitemap.xml');
        ...
        utils_1.logger.success('Sitemap generated');
        return sitemapPath;
    }
    /**
     * 生成搜索索引
    ...
    async generateSearchIndex(posts) {
        ...
        utils_1.logger.info('Generating search index...');
        await search_1.SearchIndexGenerator.generate(posts, this.config.build.outputDir);
        utils_1.logger.success('Search index generated');
    }
    /**
     * 获取生成的页面列表
     */
